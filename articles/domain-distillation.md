...
前回は 3 種類のサービスのみの対応だったのが、ビジネスの拡大に伴い 30 種類^[対応範囲が膨大となる為、旧システムでの連携もまだ残っており、法改正時点で実際連携したのは10サービスのみとなりました]のサービスまで拡大する必要がありました。また、今回の法改正のために新規で作成した Rezept as a Service で初めて法改正を迎えるという内容でした。
...
このサービスコードは請求条件を表し、コードの数だけ請求パターンが存在します。単純に考えると、前回の 3 倍のテストが必要です。^[全てのパターンを検証することは難しいため、品質を担保する工夫しています。テストケース数が3倍にならないように努めていますが、複雑度は増しています。これをご理解いただけると幸いです。]
...
まずコア業務以外の周辺業務を切り離していくことを行っていきました。例えば請求業務の中でも利用者への負担額請求は、以前のシステムでは 1 つのマイクロサービスとして定義していましたが、これは明確にレセプトの業務外として定義して分けました。
...
LITALICO は実際に支援事業を行っており環境として恵まれていると実感しています。社内にはドメインのエキスパートがおり、さらに SaaS 展開もしていてサービスを利用している事業所数が多く、ノウハウの蓄積ができています。これにより、具体的に決めていける環境があり、大変助けられました。

ただ、関係者が多くなるので、これらの議論を円滑に且つわかりやすくまとめ上げるのに苦労しました。事前にこういうフォーマットで決めていこうとしたものが、機能しなかったり改善すべき点が多かったです。
システム設計では、Architecture Decision Record（ADR）を記録します。しかし、特殊なドメイン領域なので、そのままでは使えないと考えています。
ドメイン特化の算定構造決定レコードが必要と考え、現在フォーマットの検討及び試行中です。
...
まずは、スキーマ自体の品質を向上させる為の取り組みについては [「実装と乖離させないスキーマ駆動開発フロー / OpenAPI Laravel編」](https://zenn.dev/katzumi/articles/schema-driven-development-flow) にまとめています。ぜひご覧ください。
...
１にも通じますが、依存していた実績データのパターンを考える必要はなく、I/F は契約プログラムとなります。厳密なパターンでのテストケースを作成すれば良くなりました。

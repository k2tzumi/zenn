...
レセプト基盤はマルチテナントなシステムとして構築していますが、データ管理およびシステム構成の都合から、各プロダクトをシングルユースの構成で VPC 上に構築しています。
...
実装したバージョンでいつでもセルフサービスでデプロイできること。
...
設計の原則が、アーキテクチャレベルにも適用できるということがおわかり頂けたら幸いです。
